...
public interface IUserFootprintMapper extends BaseMapper<UserFootprint> {
    ...
    @Select("select * from t_user_footprint " +
            "where user_id=#{userId} and  exper_id=#{experId}")
    List<UserFootprint> selectListByUserExper(@Param("userId") String userId, @Param("experId") String experId);

//    @Delete("delete from t_user_footprint where user_id=#{userId} and exper_id=#{experId}")
//    void deleteByUserExper(@Param("userId") String userId, @Param("experId") String experId);
